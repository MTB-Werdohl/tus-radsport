-- Phase 3 ÔÇö Profilbilder (Avatars)
-- Nach supabase-strava-public.sql und supabase-members-anonymize.sql ausf├╝hren
...
comment on function public.get_own_member_id is
  'Eigene member_id aus JWT-E-Mail; f├╝r Storage-RLS und Profil-RPCs.';
...
comment on function public.build_avatar_public_url is
  '├ûffentliche Avatar-URL f├╝r RPCs (Cache-Busting via avatar_updated_at).';
...
-- Eigenes Verzeichnis {member_id}/ÔÇª
...
-- ---------------------------------------------------------------------------
-- ├ûffentliche RPCs: avatar_url wenn avatar_storage_path gesetzt
-- (ohne einwilligung_bilder; Opt-ins publish_feed / publish_rankings unver├ñndert)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Profil-RPC: eigenes Avatar (optional, f├╝r Client ohne direkten Storage-Zugriff)
-- ---------------------------------------------------------------------------
...
comment on function public.get_member_profile_avatar is
  'Eigenes Profilbild-Metadaten f├╝r Profil-Tab (Mitglied/Vorstand).';

-- ---------------------------------------------------------------------------
-- anonymize_member: Avatar-Felder nullen + Storage-Objekt l├Âschen
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_member_id is null then
    ...
    if v_rolle <> 'public' then
      raise exception
        'Nur externe Teilnehmer k├Ânnen ihr Konto selbst l├Âschen.';
    end if;
...
comment on function public.get_public_activity_feed is
  '├ûffentlicher Feed: publish_feed, soft-delete-frei, Standard 90 Tage; avatar_url wenn gesetzt.';

comment on function public.get_public_activity_detail is
  'Aktivit├ñtsdetail per UUID; nur sichtbare Feed-Eintr├ñge; avatar_url wenn gesetzt.';
