...
**Vorstand Teilnehmerliste (Einzeltermine):** [`supabase-admin-event-participants.sql`](../supabase-admin-event-participants.sql) — RPC `admin_manage_event_participant`, `list_guest_walkin_drafts`, `members.walkin_open` / `walkin_module_id`, Rolle `guest`; nach Phase-4a-Basis.
